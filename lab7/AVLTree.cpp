...
Node *AVLTree::rightRotate(Node *np)
{
    ...
    Node *left_of_root = np->left;
    Node *right_of_left = left_of_root->right;

    left_of_root->right = np;
    np->left = right_of_left;
    np->height = 1 + max(nodeHeight(np->left), nodeHeight(np->right));
    left_of_root->height = 1 + max(nodeHeight(np->left), nodeHeight(np->right));
    return left_of_root;
        /* TODO: Students write code here */
...
Node *AVLTree::leftRotate(Node *np)
{
    ...
    Node *right_of_root = np->right;
    Node *left_of_right = right_of_root->left;
    right_of_root->left = np;
    np->right = left_of_right;
    np->height = 1 + max(nodeHeight(np->left), nodeHeight(np->right));
    right_of_root->height = 1 + max(nodeHeight(np->left), nodeHeight(np->right));
